...
def match_recipes(available_ingredients, recipes, dietary=None, max_results=8):
    ...
    for r in recipes:
        ...
        score = common - 0.5 * max(0, len(req - available))  # prefer matches with fewer missing ingredients
...
def main():
    st.title("🍽️ Smart Recipe Generator (Streamlit)")
    st.markdown("Enter the ingredients you have (comma-separated) and optional dietary preference. The app will suggest matching recipes.")

    recipes = load_recipes()

    col1, col2 = st.columns([2,1])
    with col1:
        ing_text = st.text_area("Available ingredients (comma-separated)", value="eggs, milk, flour, tomato, onion", height=80)
    with col2:
    ...
    if st.button("Find recipes"):
        if not available_ingredients:
            ...
            matches = match_recipes(available_ingredients, recipes, dietary_filter, max_results=max_results)
            if not matches:
                st.info("No matching recipes found. Try removing dietary filter or adding more ingredients.")
            else:
                st.success(f"Found {len(matches)} suggested recipes")
                for r in matches:
                    with st.expander(f"{r['title']} — {r['time_minutes']} min — Difficulty: {r['difficulty']}"):
                        st.markdown(f"**Ingredients:** {', '.join(r['ingredients'])}")
                        st.markdown(f"**Servings:** {r.get('servings',1)} — **Calories:** {r.get('nutrition',{{}}).get('calories','N/A')} kcal")
                        ...
                        with colA:
                            if st.button(f"Save favorite ❤️  {r['id']}", key=f"fav_{r['id']}"):
                        ...
                        with colB:
                            rating = st.slider(f"Rate this recipe {r['id']}", 0, 5, 0, key=f"rate_{r['id']}")
                            if rating>0:
                                st.write("Thanks for rating! (stored in-session only)")
    ...
    st.sidebar.markdown("---")
    st.sidebar.write("**About**: This is a demo Streamlit app created to satisfy the Smart Recipe Generator assignment. See README for deployment instructions.")
